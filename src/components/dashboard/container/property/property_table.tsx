...
import SelectMenu from "./component/select";
import PropertyTableContent from "./component/table";
import { dummyPropertyTable } from "./dummy/property_table_dummy";
import { useState } from "react";

export default function PropertyTable() {
    const dummyData = dummyPropertyTable;
    const [ totalPage, _setTotalPage ] = useState<number>(10);
    const [ selectedPage, setSelectedPage ] = useState<number>(1);

    return (
        <div className="w-full h-full flex flex-col items-start justify-start gap-y-4 bg-white shadow-md rounded-md p-8 px-10 overflow-hidden max-h-[470px]">
            <div className="flex flex-col gap-y-4 w-full h-full">
                ...
                <PropertyTableContent data={dummyData} />
